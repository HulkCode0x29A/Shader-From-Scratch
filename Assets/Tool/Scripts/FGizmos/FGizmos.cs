...
public class FGizmos
{
    ...
    public static void DrawFrustum(float left, float right, float bottom, float top, float zNear, float zFar, Transform obj)
    {
        FrustumData data = GetFrustumData(left, right, bottom, top, zNear, zFar);
        Matrix4x4 transMatrix = FMatrix.Translation(obj.position);
        Vector3 eulerAngle = obj.eulerAngles;
    ...
    public static void DrawWireTriangle(Vector3 p0, Vector3 p1, Vector3 p2)
    {
        Gizmos.DrawLine(p0, p1);
        Gizmos.DrawLine(p1, p2);
